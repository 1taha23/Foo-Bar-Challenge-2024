...
def solution(start, length):
    ...
    # Iterate over each row in the checkpoint line
    for i in range(length):
        # Calculate the start and end IDs for this row
        print(i)
        row_start = start + i * length
        print("start: " , row_start)
        row_end = row_start + length - i
        print("end : " , row_end)

        # Calculate the checksum for this row and XOR it with the current checksum
        checksum ^= xor_to_n(row_end - 1) ^ xor_to_n(row_start - 1)
        print(checksum)
        print("################")
